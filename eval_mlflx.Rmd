...
for (targetveg in all_vegtypes){
  ...
  write_csv(dfv, file = filn)  
} else {
  dfv <- read_csv(filn)
}

saveRDS(dfv, file = "data/bias_leave_vegtype_out_cv.csv")
```
...
gg1 <- dfv |> 
  ...
  theme_classic() +
  labs(title = "Bias on DBF sites", x = expression(paste("GPP bias (gC m"^-2, "d"^-1, ")"))) +
  scale_color_jcolors(palette = "default", name = "Trained on")
...
gg2 <- dfv |> 
  ...
  geom_vline(xintercept = 0, linetype = "dotted") +
  theme_classic() +
  labs(title = "Bias on GRA sites", x = expression(paste("GPP bias (gC m"^-2, "d"^-1, ")"))) +
  scale_color_jcolors(palette = "default", name = "Trained on")
...
gg3 <- dfv |> 
  ...
  geom_vline(xintercept = 0, linetype = "dotted") +
  theme_classic() +
  labs(title = "Bias on ENF sites", x = expression(paste("GPP bias (gC m"^-2, "d"^-1, ")"))) +
  scale_color_jcolors(palette = "default", name = "Trained on")
...
gg4 <- dfv |> 
  ...
  geom_vline(xintercept = 0, linetype = "dotted") +
  theme_classic() +
  labs(title = "Bias on MF sites", x = expression(paste("GPP bias (gC m"^-2, "d"^-1, ")"))) +
  scale_color_jcolors(palette = "default", name = "Trained on")
...
dfv |> 
  ...
  summarise(rrmse_bias = rmse(bias)) |> 
...
```{r}
## collect predictions of model trained on 'othercont', and predicting on site in 'targetcont.' 
## since this site is in 'targetcont', multiple models trained on 'othercont' predicted this site, 
...
collect_othercont_bysite <- function(targetcont, othercont, targetsite){
  ...
  purrr::map_dfr(
    as.list(list.files(path = paste0("data/leave_", targetcont, "_out"), pattern = "_out_bias")),
    ~read_delim(
...
for (targetcont in all_conttypes){
  ...
  for (targetsite in all_sites){
    ...
    if (targetcont == "US" && str_sub(targetsite, start = 1, end = 2) == "US"){
      ...
      ## collect predictions of model trained on remaining sites of the same cont (targetcont)
      dfc <- dfc |> 
        bind_rows(
          read_delim(
...
# dfv <- readRDS("data/bias_leave_cont_out_cv.csv")
...
gg1 <- dfc |> 
  filter(iscont == "US") |> 
  ggplot(aes(x = bias, y = ..density.., fill = trainedoncont, alpha = 0.5)) +
  geom_density() + 
  geom_vline(xintercept = 0, linetype = "dotted") +
  theme_classic() +
  ...
  scale_fill_jcolors(palette = "default", name = "Trained on")
...
gg2 <- tmp |>  
  ggplot(aes(x = bias, y = ..density.., fill = cont, alpha = 0.5)) +
  geom_density() + 
  geom_vline(xintercept = 0, linetype = "dotted") +
  theme_classic() +
  labs(title = "Bias of LSOCV", 
       ...
       x = expression(paste("GPP bias (gC m"^-2, "d"^-1, ")"))) +
  scale_fill_jcolors(palette = "default", name = "Continent")

plot_grid(gg1, gg2, ncol = 2, labels = c("a", "b"))
ggsave("fig/leave_veg_out_cv.pdf", width = 10, height = 3)
```
