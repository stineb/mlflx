...
## leave-DBF-out CV: Collect all data predicted by model trained on all but DBF data
vegtype <- "DBF"
list.files(path = paste0("data/leave_", vegtype, "_out"))

sitenames <- siteinfo_fluxnet2015$sitename %>% unique()

for (idx in seq(nrow(siteinfo_fluxnet2015))){
  sitename <- siteinfo_fluxnet2015$sitename[idx]
  vegtype  <- siteinfo_fluxnet2015$classid[idx]
