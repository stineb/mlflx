...
gg1 <- df_lsocv_metrics %>% 
  ggplot(aes(rsq_null, rsq_lstm, label = sitename)) +
...
gg2 <- df_lsocv_metrics %>% 
  ggplot(aes(rsq_null, rsq_dnn, label = sitename)) +
  geom_point(color = "red") +
  xlim(0,1) + ylim(0,1) +
  geom_abline(yintercept = 0, slope = 1, linetype = "dotted") +
  theme_classic() +
...
df %>% 
  ...
  rename(lstm = LSTM_de, dnn = DNN_de, null = gpp_null, obs = GPP_NT_VUT_REF) %>% 
  pivot_longer(c(obs, lstm, dnn, null),
               names_to = "model", 
               values_to = "gpp") %>% 
  ggplot(aes(date, gpp, color = model)) +
  scale_color_manual(
    values = c("dnn" = "royalblue", "lstm" = "red", "obs" = "black", "null" = "grey70"),
    labels = c("DNN", "LSTM", "obs.", "NULL")
    ) +
  geom_line() +
...
```{r}
df %>% 
  dplyr::filter(sitename == "AU-Whr" 
                # & lubridate::year(date) %in% c(2013)
                ) %>% 
  rename(lstm = LSTM_de, dnn = DNN_de, null = gpp_null, obs = GPP_NT_VUT_REF) %>% 
  pivot_longer(c(obs, lstm, dnn, null),
               names_to = "model", 
...
df2 <- df %>%
  ungroup() %>% 
  # mutate(bias_lstm = LSTM_de - GPP_NT_VUT_REF,
  #        bias_dnn = DNN_de - GPP_NT_VUT_REF,
  #        bias_null = gpp_null - GPP_NT_VUT_REF) %>% 
  mutate(bias_lstm = abs(LSTM_de - GPP_NT_VUT_REF),
         bias_dnn = abs(DNN_de - GPP_NT_VUT_REF),
         bias_null = abs(gpp_null - GPP_NT_VUT_REF)) %>% 
  mutate(bin = cut(wscal, breaks = nbin)) %>% 
