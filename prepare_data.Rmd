...
Write to file.
```{r}
write_csv(ddf, path = "data/ddf_fluxnet_mlflx_20210507.csv")
save(ddf, file = "data/ddf_fluxnet_mlflx_20210507.RData")
```
...
missing_sites_soilm <- df_sites$sitename[!(df_sites$sitename %in% unique(df_soilm$sitename))]
missing_sites_soilm
...
write_csv(ddf, path = "data/ddf_fluxnet_mlflx_wscal_20210505.csv")
save(ddf, file = "data/ddf_fluxnet_mlflx_wscal_20210505.RData")
```
...
Get MOD09A1 reflectance data from [ORNL DAAC](https://lpdaac.usgs.gov/products/mod09a1v006/).
...
df_modis_fpar <- ingest(
  df_sites_ddf,
    # mutate(year_start = 2001, year_end = 2019),
  source = "modis",
...
Write to file.
```{r}
write_csv(df_modis_fpar, path = "data/ddf_fpar_modis_mlflx_20210505.csv")
save(df_modis_fpar, file = "data/ddf_fpar_modis_mlflx_20210505.RData")
```
...
calc_frac_missing <- function(df){
  (df %>% dplyr::filter(is.na(fpar)) %>% nrow(.))/nrow(df)
}
sites_missing_fapar2 <- ddf_mlflx %>% 
  group_by(sitename) %>% 
  nest() %>% 
  mutate(frac_missing = purrr::map_dbl(data, ~calc_frac_missing(.))) %>% 
  arrange(desc(frac_missing)) %>% 
  dplyr::filter(frac_missing > 0.1) %>% 
  pull(sitename) %>% 
  unique()

ddf_mlflx <- ddf_mlflx %>% 
  dplyr::filter(!(sitename %in% sites_missing_fapar2))
